...
class CinepakDecoder {
  public:
...
#ifdef USE_DRAW_CALLBACK
    void decodeFrame(uint8_t* data, size_t data_size, uint16_t* output_buf, size_t output_buf_size, DRAW_CALLBACK* draw,
                     bool iskeyframe)
#else
...
#endif
    {
      ...
      // The theory behind this is that this is here to confuse standard Cinepak decoders. But, we won't let that
      // happen! ;)
