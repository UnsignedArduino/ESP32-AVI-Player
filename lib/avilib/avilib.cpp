...
static size_t avi_read(File fd, char* buf, size_t len) {
  size_t n = 0;
  size_t r = 0;

  while (r < len) {
    n = fd.read((uint8_t*)(buf + r), len - r);
...
static size_t avi_write(File fd, char* buf, size_t len) {
  size_t n = 0;
  size_t r = 0;

  while (r < len) {
    n = fd.write((uint8_t*)(buf + r), len - r);
    if ((ssize_t)n < 0)
      return n;
...
#else
  AVI->fdes = SD.open(filename, FILE_WRITE);
#endif
  if (!AVI->fdes) {
...
#define AVIF_COPYRIGHTED 0x00020000
  ...
  if (!AVI->fdes.seek(0) || avi_write(AVI->fdes, (char*)AVI_header, HEADERBYTES) != HEADERBYTES ||
      !AVI->fdes.seek(AVI->pos)) {
...
#else
  AVI->fdes = SD.open(filename, FILE_READ);
#endif
  if (AVI->fdes < 0) {
    AVI_errno = AVI_ERR_OPEN;
    free(AVI);
...
int avi_parse_input_file(avi_t* AVI, int getIndex) {
  ...
  while (1) {
    ...
    if (strncasecmp(data, "LIST", 4) == 0) {
      ...
      if (strncasecmp(data, "hdrl", 4) == 0) {
        ...
        header_offset = AVI->fdes.position();

        if (avi_read(AVI->fdes, (char*)hdrl_data, n) != n)
          ERR_EXIT(AVI_ERR_READ)
      } else if (strncasecmp(data, "movi", 4) == 0) {
        AVI->movi_start = AVI->fdes.position();
        AVI->fdes.seek(AVI->fdes.position() + n);
      } else
        AVI->fdes.seek(AVI->fdes.position() + n);
    ...
    if (strncasecmp((char*)hdrl_data + i, "strh", 4) == 0) {
      ...
      if (lasttag == 1) {
        ...
        if (wfe != NULL) {
          ...
          if (wfe->cb_size != 0) {
            nwfe = (char*)realloc(wfe, sizeof(WAVEFORMATEX_avilib) + wfe->cb_size);
            if (nwfe != 0) {
              off_t lpos = AVI->fdes.position();
              AVI->fdes.seek(header_offset + i + sizeof(WAVEFORMATEX_avilib));
  ...
  if (AVI->idx) {
    ...
    if (strncasecmp(data, (char*)AVI->idx[i], 4) == 0 && str2ulong((unsigned char*)data + 4) == len) {
      ...
      AVI->fdes.seek(pos + AVI->movi_start - 4);
      if (avi_read(AVI->fdes, data, 8) != 8)
        ERR_EXIT(AVI_ERR_READ)
      if (strncasecmp(data, (char*)AVI->idx[i], 4) == 0 && str2ulong((unsigned char*)data + 4) == len) {
  ...
  if (idx_type == 0) {
    ...
    while (1) {
      ...
      if (strncasecmp(data, "LIST", 4) == 0) {
        AVI->fdes.seek(AVI->fdes.position() + 4);
        continue;
      }
      ...
      if (((data[2] == 'd' || data[2] == 'D') &&
        ...
        avi_add_index_entry(AVI, (unsigned char*)data, 0, AVI->fdes.position() - 8, n);
      }

      AVI->fdes.seek(PAD_EVEN(n));
    }
    idx_type = 1;
  }
...
long AVI_read_frame(avi_t* AVI, char* vidbuf, int* keyframe) {
  ...
  *keyframe = (AVI->video_index[AVI->video_pos].key == 0x10) ? 1 : 0;
  ...
  AVI->fdes.seek(AVI->video_index[AVI->video_pos].pos);
  ...
  while (1) {
    ...
    if (strncasecmp(data, AVI->video_tag, 3) == 0) {
      ...
      *len = n;
      ...
      return 2;
      break;
    } else if (AVI->fdes.seek(AVI->fdes.position() + n))
      return 0;
  }
}
